...
async fn full_case_lifecycle() {
    ...
    let id = "e2e-0001";
    // in case a previous run left traces
    ...
    // the client (scoped key) drops evidence under data/
    ...
    // the team retrieves (data/ prefix only)
    ...
    // teardown: the single bucket is deleted
